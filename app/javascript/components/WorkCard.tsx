    ...
    <Card className="mb-3" id={`work-${work.id}`}>
      <Card.Body>
        <Card.Text className="d-flex justify-content-between align-items-start mb-1">
          {work.kadai ? (
            <a
              href={`/${work.kadai.year}/${work.kadai.round}`}
            ...
            <a
              href={`/students/${work.student.genron_sf_id || work.student.id}`}
              className="link-secondary"
            >
              {work.student.name}
            </a>
        ...
        <Card.Text as="ul" className="list-inline">
          {voters.map((voter) => (
            <li key={voter.twitter_id} className="list-inline-item mr-0">
              <OverlayTrigger
                overlay={
                  <Tooltip id={`${work.id}-${voter.twitter_id}`}>
                    {voter.twitter_screen_name}
                  </Tooltip>
                }
              >
                <a
                  href={`https://twitter.com/${voter.twitter_screen_name}`}
                  target="_blank"
                  rel="noreferrer"
                >
                  <Image
        ...
        <WorkVoteButton
          ...
          isVoted={
            currentUser
              ? voters.some(
                  (voter) => voter.twitter_id === currentUser.twitter_id,
                )
              : false
          }
